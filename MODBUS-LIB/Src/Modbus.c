...
const osThreadAttr_t myTaskModbusA_attributes = {
    .name = "myTaskModbusA",
    .priority = (osPriority_t) osPriorityNormal | portPRIVILEGE_BIT,
    .stack_size = 128 * 4
};
...
void ModbusInit(modbusHandler_t * modH)
{
  ...
  if (numberHandlers < MAX_M_HANDLERS)
  {
    ...
    RingClear(&modH->xBufferRX);
	  ...
	  //hQueue = xQueueCreate (MAX_BUFFER, sizeof(uint8_t));
    ...
    //  modH->QueueModbusHandle = hQueue;
	  ...
	  if(modH->uiModbusType == SLAVE_RTU)
	  {
		  ...
		  #if PRIV
		  prio = prio | portPRIVILEGE_BIT;
		  #endif

		  #if MPU_EN
			  //if we using MPU
			  static StackType_t xModbusTaskStack[ STACK_SIZE ] __attribute__( ( aligned( STACK_SIZE * sizeof( StackType_t ) ) ) );
			  TaskParameters_t xModbusTaskParameters =
			  {
			  	.pvTaskCode		= StartTaskModbusSlave,
			  	.pcName			= "myTaskModbusA",
			  	.usStackDepth	= STACK_SIZE,
			  	.pvParameters	= ucSharedMemory, //this is where modH will be
			  	.uxPriority		= prio,
			  	.puxStackBuffer	= xModbusTaskStack,
			  	.xRegions		=	{
			  							{ ucSharedMemory,	SHARED_MEMORY_SIZE,	portMPU_REGION_READ_WRITE | portMPU_REGION_EXECUTE_NEVER},
			  							{ xModbusShareRegion,		MSR_SIZE,	portMPU_REGION_READ_WRITE | portMPU_REGION_EXECUTE_NEVER},
			  							{ 0,	0,	0},
			  						}
				#if BOX
			  ...
			  if (MPU_xTaskCreateRestricted( &( xModbusTaskParameters ), &hTask ) != pdPASS){
				  hTask = NULL;
			  }
			#else
			  //if MPU is disabled
...
void StartTaskModbusSlave(void *argument)
{
  ...
  xPeripheralRequest.ulAddress = (uint32_t)modH->au8Buffer;
  xPeripheralRequest.ulSize = sizeof(modH->au8Buffer);
...
void sendTxBuffer(modbusHandler_t *modH)
{
	...
	xPeripheralRequest.ulAddress = (uint32_t)modH->au8Buffer; // source address
	xPeripheralRequest.ulSize = modH->u8BufferSize;  //size of transfer
	xPeripheralRequest.ulRegionNumber = portFIRST_REGION_NUMBER;
	xPeripheralRequest.ulOption1 = OPT_NULL;
	if( MPU_xContainerSendRequest(xPeripheralRequest) == pdFREERTOS_ERRNO_EACCES  )
	{
